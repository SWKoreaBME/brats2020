...
class UNETR(nn.Module):
    ...
    def __init__(
        ...
        self.num_layers = num_layers  # default: 12
...
if __name__=="__main__":
    ...
    feature_size = os.environ["FEATURE_SIZE"] if not debug else 16
    hidden_size = os.environ["HIDDEN_SIZE"] if not debug else 64
    num_heads = os.environ["NUM_HEADS"] if not debug else 8
    ...
    model = UNETR(**model_args).to(torch.device("cuda"))
    x = torch.randn(2, 4, 240, 240).to(torch.device("cuda"))
