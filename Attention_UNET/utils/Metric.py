...
def compute_meandice_multilabel(seg_out, seg_label, include_background=False, sigmoid=True):
    ...
    for label_idx in range(num_labels):
        if (not include_background) and (label_idx == 0):
            continue
        _outs, _label = seg_out[:, label_idx], seg_label[:, label_idx]
        temp_dice = get_dice_score(_outs.detach().cpu(), _label.detach().cpu()).nanmean()
        seg_batch_score = seg_batch_score + temp_dice
        A.extend([temp_dice.cpu().numpy().item()])
        if torch.isnan(seg_batch_score).any():
            print(seg_batch_score)
    if include_background:
        seg_batch_score /= num_labels
...
class Score:
    def __init__(self, include_background=False, sigmoid=False, softmax=False):
        ...
        self.total_indv_dicescores = np.array([0,0,0],dtype=np.float64)
    ...
    def save(self, ckpt_save_dir):
        ...
        indv_dice_score = self.total_indv_dicescores / self.total_num_imgs

        conf_matrix_sum = self.total_conf_matrix.sum(0)
        
        mean_sen, mean_spe = 0, 0
        class_spe = np.array([0,0,0],dtype=np.float64)
        class_sen = np.array([0,0,0],dtype=np.float64)
        for label_idx in range(conf_matrix_sum.size(0)):
            sen, spe = self.get_sen_spe(conf_matrix_sum[label_idx])
            mean_sen += sen
            mean_spe += spe
            class_sen[label_idx] +=sen
            class_spe[label_idx] +=spe
            
        mean_sen = mean_sen / len(self.tumor_names)
        mean_spe = mean_spe / len(self.tumor_names)
        
        print(f"Average Total Dice: {dice_score:.4f}")
        print(f"Average Total Sensitivity: {mean_sen:.4f}")
        print(f"Average Total Specificity: {mean_spe:.4f}")
        print(f"Per Class Average Dice Score: {indv_dice_score}")
        print(f"Per Class Specificity: {class_spe}")
        print(f"Per Class Sensitivity: {class_sen}")
    ...
    def evaluate(self, y_pred, y_true):
        ...
        dice_score,indv_dicescore = compute_meandice_multilabel(y_pred,
                                                 y_true, 
                                                 include_background=self.include_background, 
                                                 sigmoid=False)
        dice_score = dice_score * y_pred.size(0)
        indv_dicescore = np.array(indv_dicescore) * y_pred.size(0)
        ...
        self.total_indv_dicescores+=indv_dicescore
